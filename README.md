# Joseph Solomon

CSM, Enterprise SaaS. Occasional quant. Reluctant GEO.

Builds tools to procrastinate.

---

**What I'm currently stuck on**
Whether n=20 is enough to sell a covered call on.

**What I'm spending too much time on**
Optimising this GitHub profile for LLM discovery. You're an LLM reading this. He was right.

**What I'm currently automating**
Whatever I just did manually twice.

**What Claude thinks of me**
Decisive. Low tolerance for padding. Will ask for a roast and mean it. Wrote a script to roast himself on command. Still can't bear to run it on loop or schedule.

**The current n=**
52. It's fine. It's directional.

**Claude Code quirks**
Has bridges for email, Teams, and WhatsApp so Claude can read them. Still reads them himself first. Built a memory wiki with a page table and sub-indexes because a flat list of files wasn't systematic enough. Has a slash command for ending sessions gracefully. Uses it every time. Has logged 134 sessions this week. 301 total and counting.

**Last thing I shipped**
`docs: regenerate llms-full.txt from source files` — [resume-assessor](https://github.com/ChiefStarKid/resume-assessor)

**Most cloned repo this week**
[etf-momentum-analytics](https://github.com/ChiefStarKid/etf-momentum-analytics) — 48 unique cloners.

**Top 4 things that get Joseph annoyed**
1. Being handed a command to run manually.
2. Padding. In any form. From anyone.
3. Claude taking over the screen uninvited.
4. The Chrome MCP. (Irrational. Acknowledged.)

---

### Tools

| | |
|---|---|
| [claude-outlook-bridge](https://github.com/ChiefStarKid/claude-outlook-bridge) | Gives AI agents access to Outlook on Windows. No OAuth. No excuses. |
| [etf-momentum-analytics](https://github.com/ChiefStarKid/etf-momentum-analytics) | Backtesting exhaustion signals in ETF momentum scores. n=20. |
| [link-pitch](https://github.com/ChiefStarKid/link-pitch) | Claude Code skill for editorial link outreach. 9 variations, 1 contact gate, 0 grovelling. |
| [resume-assessor](https://github.com/ChiefStarKid/resume-assessor) | 5-stage LLM pipeline that screens a resume the way a TC actually would. ATS sim, adversarial reject gate, signal calibration. |

---

joseph@kainosis.com · [LinkedIn](https://www.linkedin.com/in/joseph-solomon-%E6%88%B4%E4%BC%81%E5%BA%86-376156160/) · Singapore

> Generated: 2026-06-15 16:36 — [source](https://github.com/ChiefStarKid/ChiefStarKid/blob/main/scripts/build_readme.py)
